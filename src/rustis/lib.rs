#[desc = "A hopefully functional and useful redis client"];
#[license = "MIT"];
...
impl Client {
  ...
  fn parse_response(&mut self) -> ~str {
  ...
  fn parse_status(&mut self) -> ~str {
    replace(self.stream.read_line().unwrap(), "\r\n", "")
  }
